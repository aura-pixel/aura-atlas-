...
export function UnitImage({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={!isEditable}
      className={`group relative flex h-[230px] w-full max-w-[300px] overflow-hidden rounded-[2rem] border border-dashed border-[#D8DCE3] bg-[#FAFAFB] text-left transition-all duration-300 ${
        isEditable
          ? "cursor-pointer hover:-translate-y-1 hover:border-[#CFC7FF] hover:bg-white hover:shadow-lg"
          : "cursor-default"
      }`}
      style={{
        background: imageUrl
